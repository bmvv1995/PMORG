...
| `PLT-005` | Fiecare build SHALL fixa `onyx_surface: ce|ee` și `usage_mode: development_test|production` într-un manifest detașat peste un catalog așteptat și set exact, canonic, fără missing/unexpected/duplicate. Payloadurile SHALL fi semnate prin DSSE fără autoreferință, iar toate rapoartele, trust material și evidence SHALL exista ca bytes content-addressed verificabili offline. Două builduri curate independente SHALL produce aceiași descriptori și aceleași artifact-set/image-lock/qualification/report payload hashes. `ce` SHALL exclude EE; orice `ee` SHALL avea inventar complet. |
| `PLT-006` | Catalogul capabilităților SHALL acoperi exact setul cerințelor aplicabile release-ului, cu zero unmapped/unknown și exact un disposition record per capabilitate. Candidate search SHALL fixa corpusul Onyx, commitul, suprafețele, planul, rezultatele brute și clasificarea hiturilor; `candidates=[]` SHALL fi permis numai pentru `pmorg_independent` demonstrat prin search evidence complet. O capabilitate Onyx `pass` SHALL fi reutilizată implicit; `patch` sau `pmorg_independent` în prezența sa SHALL cere ADR/waiver valid. `licensed_patch` SHALL desemna exclusiv un patch direct într-o cale upstream EE, cu ownership Onyx Enterprise și patch-ledger owner exact; SHALL fi interzis pentru căi PMORG-owned și builduri `ce`. Match/search evidence SHALL fi byte-closed în qualification bundle, iar codul EE SHALL NOT fi copiat în module PMORG. |
| `PLT-007` | Orice deploy, startup și revalidation SHALL reconstrui din runtime/OCI/bytes descriptorul payloadului efectiv și descriptorul/fingerprint-ul canonic al țintei, apoi SHALL valida payload + DSSE pentru measurement și `DeploymentAdmissionRecord`, legate de artifact set, payload, target, suprafață, mod, evidence offline și timp trusted. Orice `development_test`, CE sau EE, SHALL admite numai sandbox sintetic; `ce + production` SHALL cere release admission, iar `ee + production` SHALL cere suplimentar autorizare Enterprise. Unknown/unmeasurable, unqualified payload, missing, not-yet-valid, expired, revoked, missed revalidation, clock failure/skew, mismatch sau verifier neacceptat SHALL refuza și quiesce fail-closed. |
| `PLT-008` | Orice registry publish ori artifact export SHALL reconstrui din bytes payloadul exact și din APIs trusted descriptorul/fingerprint-ul destinației, apoi SHALL valida measurement + `DistributionAdmissionRecord` DSSE. Orice `development_test`, CE sau EE, SHALL permite numai o destinație sintetică controlată; `production` SHALL urma aceeași separare CE/EE și aceeași autorizare aplicabilă ca deploymentul. Unknown/unmeasurable, redirect/principal/destination change, payload mismatch, evidence/time/revocation failure SHALL refuza înaintea primului byte; transferul activ SHALL revalida și aborta/quarantina înainte de a traversa deadline-ul. |
