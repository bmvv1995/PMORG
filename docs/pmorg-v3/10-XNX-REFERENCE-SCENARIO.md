...
- `XNX-M0`: traseul rapid, fără defecte, pentru Gate D;
- `XNX-LONG`: 30 de zile virtuale, cu tăcere, duplicate, restarturi,
  conflict, contradicție, supersession și recovery, pentru Gate F.
...
| `id.andrei_pop` | Andrei Pop | operations manager / validator | validează cauza, aprobă taskul de reconciliere și outcome-ul |
...
- Mihai nu își validează propriul claim;
- agentul nu validează claims generate de el;
- Mara aprobă planul, dar Andrei validează cauza și rezultatul;
...
constraints:
  ...
  - cauza și outcome-ul cer validator independent
```
...
| `R-XNX-VALID-01` | validation receipt Andrei | cauza este confirmată și necesită reconciliere formală |
...
|---|---|---|---|
| `C-XNX-01` | `observation` | numărătoarea fizică în Stock este 8 | `proposed → under_review → validated` |
| `C-XNX-02` | `fact` | două cutii au fost mutate fizic în Quality | `proposed → under_review → validated` |
| `C-XNX-03` | `hypothesis` | transferul a fost deja înregistrat în Odoo | `proposed → under_review → rejected` |
| `C-XNX-04` | `fact` | nu există transfer intern înregistrat | `proposed → under_review → validated → superseded` |
| `C-XNX-05` | `fact` | cauza este mutarea fizică neînregistrată a 2 cutii ude | `proposed → under_review → validated` |
| `C-XNX-06` | `commitment` | Mihai va înregistra transferul până la termen | `proposed → under_review → validated` |
| `C-XNX-07` | `fact` | transferul intern este înregistrat și finalizat | `proposed → under_review → validated`; supersede `C-XNX-04` |
| `C-XNX-08` | `fact` | starea fizică și Odoo corespund după control | `proposed → under_review → validated` |
...
În M3, „cred” produce hypothesis, nu fact. Răspunsul M5 și live read-ul susțin
claim-ul validabil, dar Andrei rămâne validatorul independent.
...
6. Live read produce E3; C3 intră `under_review`, contradiction este
   înregistrată, iar C3 este respins.
7. Andrei validează C1, C2, C4 și C5.
...
11. La tickul de control, taskul VERIFY este revendicat, E7 susține C8 și
    Andrei verifică outcome-ul.
...
| 2026-01-14 09:00 | 2 zile lucrătoare fără răspuns; M2 trimis o singură dată |
| 2026-01-19 09:00 | încă 3 zile lucrătoare; escaladare la Mara; fără al doilea follow-up |
...
| 2026-01-20 09:00 | Andrei validează cauza; se creează plan candidate `P-XNX-02` cu task spec și `state_version=1` |
| 2026-01-20 09:15 | Mara aprobă command hash-ul de activare bazat pe `P-XNX-02@1` |
| 2026-01-20 09:30 | Mara modifică manual termenul în plan candidate; acesta devine `P-XNX-02@2`; comanda aprobată pe versiunea 1 primește `VERSION_CONFLICT` și zero efecte |
| 2026-01-20 10:00 | PMORG recitește starea și cere approval nou pentru noul command hash |
...
| 2026-02-11 09:00 | starea este reconstruită; un run nou revendică VERIFY; Andrei validează C8/outcome; taskul și inițiativa se închid |
...
- Mihai încearcă să-și valideze claim-ul;
